...
@section('content')
    <div class="mb-4 flex justify-center">
        <img
            src="{{ asset('images/LogoSinFondo.png') }}"
            alt="SpaProgram"
            class="h-40 w-auto"
        />
    </div>
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Ingresá con tu email y contraseña.</p>
        </div>

        <button
            type="button"
            data-theme-toggle
            class="inline-flex items-center justify-center rounded-lg bg-white p-2 text-slate-700 hover:bg-slate-50 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-900"
            aria-pressed="false"
        >
            <img
    ...
    <form method="POST" action="{{ route('login.store', [], false) }}" class="mt-6 space-y-4">
        ...
        <div>
            ...
            <input
                ...
                required
                value="{{ old('email') }}"
                class="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-slate-200"
            />
            @error('email')
        ...
        <!-- <div class="rounded-xl bg-slate-50 p-3 text-xs text-slate-600 dark:bg-slate-900/40 dark:text-slate-300">
            <div class="font-semibold text-slate-700 dark:text-slate-200">Usuarios demo (desarrollo)</div>
            <div class="mt-1">
