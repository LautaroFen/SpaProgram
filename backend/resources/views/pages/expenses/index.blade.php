                ...
                <table class="table-grid min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50 dark:bg-slate-900">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300">Categoría</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300">Proveedor</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300">A pagar</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300">Pagado</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-sm text-slate-500 dark:text-slate-400">No hay expensas para mostrar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="w-full rounded-2xl bg-white shadow-xl dark:bg-slate-950">
                ...
                <form method="post" action="{{ route('expenses.store', [], false) }}" class="px-5 py-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label for="category" class="block text-sm font-semibold text-slate-700">Categoría</label>
                            <input id="category" name="category" value="{{ old('category') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900" placeholder="materiales, impuestos..." />
                        </div>

                        <div>
                            <label for="payee" class="block text-sm font-semibold text-slate-700">Proveedor / servicio</label>
                            <input id="payee" name="payee" value="{{ old('payee') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900" placeholder="AFIP, proveedor, alquiler..." />
                        </div>

                        <div>
                            <label for="amount_due" class="block text-sm font-semibold text-slate-700">Cantidad a pagar</label>
                            <input id="amount_due" name="amount_due" type="number" min="0" step="0.01" value="{{ old('amount_due') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900" />
                        </div>

                        <div>
                            <label for="amount_paid" class="block text-sm font-semibold text-slate-700">Cantidad pagada</label>
                            <input id="amount_paid" name="amount_paid" type="number" min="0" step="0.01" value="{{ old('amount_paid') }}" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900" />
                        </div>

                        <div class="sm:col-span-2">
                            <label for="performed_at" class="block text-sm font-semibold text-slate-700">Fecha de realización</label>
                            <input id="performed_at" name="performed_at" type="datetime-local" value="{{ old('performed_at') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900" />
                        </div>
                    </div>
